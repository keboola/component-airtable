...
class CachedOrthogonalDictWriter:
    ...
    def __init__(self, file_path, fieldnames, temp_directory=None, dialect="kbc", buffering=io.DEFAULT_BUFFER_SIZE,
                 *args, **kwds):
        """
        ...
        if not temp_directory:
            temp_directory = os.path.join(
                os.path.dirname(file_path), 'csv_temp')
    ...
    def _get_or_add_cached_writer(self, columns):
        ...
        if not wr:
            ...
            t_file = open(tmp_file, 'wt+', newline='',
                          buffering=self.buffering, encoding=self.encoding)
            self._tmp_file_cache[writer_key] = t_file
            wr = DictWriter(t_file, self.fieldnames.copy(),
                            *self.args, **self.kwds)
            wr.writeheader()
    ...
    def close(self):
        ...
        if len(self._writer_cache) == 1:
            ...
            self._append_missing_rows_and_close(
                final_writer, self._writer_cache, final_writer_key)
